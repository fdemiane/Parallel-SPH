  ...
  for (int i = co->start; i < co->finish; i++)
    {
      ...
      vector<Particle*> ne = octree.searchOctree(particleArray[i]);
      ...
      for (int j = 0; j < ne.size(); j++)
	{
	  ...
	  double distSquared = (ne[j]->position.x - X)*(ne[j]->position.x - X)+
		 (ne[j]->position.y - Y)*(ne[j]->position.y - Y)+
	         (ne[j]->position.z - Z)*(ne[j]->position.z - Z);
	  ...
	  if (distSquared < tolerance*tolerance) {
	    //cout << "i: "<<i << " j: " << j << endl;
	    particleArray[i].neighbors.push_back(ne[j]);
	    ...
	    particleArray[i].density += ne[j]->mass * Poly6Kernel(sqrt(distSquared), H);
      ...
      particleArray[i].pressure = PMUL * (particleArray[i].density - 1);//cout << particleArray[i].pressure << endl;
    }
...
void* calculateFlux(void* ceo)
{
  ...
  for (int i = co->start; i < co->finish; i++)
      ...
      for (int j = 0; j < particleArray[i].neighbors.size(); j++)
	{
	  ...
	  vec3 fv;
	  fv.x += ne.mass * (ne.velocity.x / ne.density)*SpikyLaplacian(rij.getMagnitude(), H)*no.x;
	  fv.y += ne.mass * (ne.velocity.y / ne.density)*SpikyLaplacian(rij.getMagnitude(), H)*no.y;
	  fv.z += ne.mass * (ne.velocity.z / ne.density)*SpikyLaplacian(rij.getMagnitude(), H)*no.z;
      ...
      particleArray[i].acceleration = particleArray[i].FGravity + particleArray[i].Fpressure+particleArray[i].Fviscocity*0.00001f;
...
int main(int argc, char** argv)
{
  ...
  for (int y = 0; y < SIZE; y++)
	...
	particleArray.push_back(Particle(x*0.1f+(rand()%10)*0.01f, y*0.1f+(rand()%10)*0.01f, z*0.1f+(rand()%10)*0.01f, 0.01, 20));

  int pL = particleArray.size();
  int2 co[16];

  octree.init(threadCount);
  for (step = 0; step < 100; step++)
    {// find neighbors

      octree.buildOctree(particleArray, threadCount);
	...
	if(isnan(particleArray[i].position.x)|| particleArray[i].position.x <-1000 || particleArray[i].position.x>1000)
	  particleArray[i].position.x = 10000;
	if(isnan(particleArray[i].position.y)|| particleArray[i].position.y <-1000 || particleArray[i].position.y>1000)
	  particleArray[i].position.y = 10000;
	if(isnan(particleArray[i].position.z)|| particleArray[i].position.z <-1000 || particleArray[i].position.z>1000)
	  particleArray[i].position.z = 10000;
	file << i << " " << particleArray[i].position.x << " " << particleArray[i].position.y << " " << particleArray[i].position.z << endl;
	particleArray[i].neighbors.clear();
      }


       octree.Clean(threadCount);
