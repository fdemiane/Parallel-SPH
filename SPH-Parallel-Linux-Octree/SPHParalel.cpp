...
void calculateDensity(void* ceo)
{
  ...
  for (int i = co->start; i < co->finish; i++)
      ...
      for (int j = 0; j < particleArray.size(); j++)
	{
      ...
      particleArray[i].pressure = 1 * (particleArray[i].density - 1);//cout << particleArray[i].pressure << endl;
    }
...
int main()
{
  ...
  for (step = 0; step < 100; step++)
      ...
      for (int i = 0; i < 4; i++)
	{
	  ...
	  co[i].start = i * pL;
	  co[i].finish = co[i].start + pL / i;
	  pthread_create(&threads[i], NULL, calculateDensity, (void*)co[i]);
	}
      ...
      for (int i = 0; i < 4; i++)
	{
	  ...
	  co[i].start = i * pL;
	  co[i].finish = co[i].start + pL / i;
	  pthread_create(&threads[i], NULL, calculateFlux, (void*)co[i]);
	}
      ...
      for (int i = 0; i < 4; i++)
	{
	  ...
	  co[i].start = i * pL;
	  co[i].finish = co[i].start + pL / i;
	  pthread_create(&threads[i], NULL, LeapFrogIntegration, (void*)co[i]);
	}
...
void LeapFrogIntegration(int2 co)
{
  for (int i = co.start; i < co.finish; i++)
    {
